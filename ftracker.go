...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedInMinSec := math.Pow(meanSpeed(action, duration), 2)

	return ((walkingCaloriesWeightMultiplier*weight + (meanSpeedInMinSec/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
